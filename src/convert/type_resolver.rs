use std::iter;
use TypeResolverError::NotFoundByAbsPath;

use crate::convert::WithFullName;
use crate::model;
...
impl<'a> LookupScope<'a> {
    ...
    fn enums(&self) -> &'a [WithLoc<model::Enumeration>] {
        match self {
            ...
            LookupScope::Message(message, _) => &message.enums,
        }
    }
...
impl<'a> TypeResolver<'a> {
    ...
    pub(crate) fn find_message_or_enum_by_abs_name(
        ...
        Err(NotFoundByAbsPath(absolute_path.clone()).into())
    }

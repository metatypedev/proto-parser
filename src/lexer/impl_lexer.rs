...
use crate::lexer::{
    ...
    jsonnumberlit::JsonNumberLit,
    loc::{Loc, FIRST_COL},
    strlit::{StrLit, StrLitDecodeError},
...
impl<'a> Lexer<'a> {
    ...
    fn next_octal_digit(&mut self) -> LexerResult<u32> {
        self.next_char_expect(|c| c.is_ascii_digit(), LexerError::ExpectOctDigit)
            .map(|c| c as u32 - '0' as u32)
    }

    // decimalDigit = "0" … "9"
    fn next_decimal_digit(&mut self) -> LexerResult<u32> {
        self.next_char_expect(|c| c.is_ascii_digit(), LexerError::ExpectDecDigit)
            .map(|c| c as u32 - '0' as u32)
    }
